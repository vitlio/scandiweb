...
class CartStorage extends PureComponent {
  ...
  static addToLocalStorageCart(action) {
    ...
    if (localStorage.cart) {
      if (
        JSON.parse(localStorage.cart).find(
          (i) =>
            i.name === action.name &&
            JSON.stringify(i.choosenSize.sort()) ===
              JSON.stringify(action.choosenSize.sort())
        )
      ) {
        let index = JSON.parse(localStorage.cart).findIndex(
          (i) =>
            i.name === action.name &&
            JSON.stringify(i.choosenSize.sort()) ===
              JSON.stringify(action.choosenSize.sort())
        ...
        action = {
          ...action,
          countInCart:
            JSON.parse(localStorage.cart)[index].countInCart +
            action.countInCart,
        };
      ...
      return localStorage.setItem(
        "cart",
        JSON.stringify([...JSON.parse(localStorage.cart), action])
      );
    }
  ...
  static changeAttInCart(action, attributes) {
    ...
    let index = arr.findIndex(
      (i, idx) => i.name === action.name && idx === attributes[2]
    );

    arr[index].choosenSize.map(
      (att) => att[0] === attributes[0] && (att[1] = attributes[1])
    );
    return localStorage.setItem("cart", JSON.stringify(arr));
  }
  ...
  static addShopCartButton__indicator() {
    let count;
    if (localStorage.cart) {
      count = JSON.parse(localStorage.cart).reduce(
        (summ, i) => (summ += i.countInCart),
        0
      );
